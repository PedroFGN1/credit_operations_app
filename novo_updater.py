...
def _atualizar_dados_siconfi(modelo_db, endpoint, params_base, periodo_params):
    ...
    chaves_unicas = {
        'RREO': ['exercicio', 'periodo', 'instituicao', 'anexo', 'rotulo', 'coluna', 'cod_conta'],
        ...
        try:
            ...
            for i in range(0, len(itens_filtrados), tamanho_lote):
                ...
                for chave_tupla in chaves_api_lote:
                    filtros_db.append(and_(*(
                        getattr(modelo_db, nome_chave) == valor for nome_chave, valor in zip(chaves_identificadoras, chave_tupla) if valor is not None
                    )))
                ...
                if i == 0:
                    print(f"Chaves existentes no DB: {len(chaves_existentes)}")
                ...
                if registros_novos_para_inserir:
                    ...
                    logger.info("Iniciando inserção em lote...")  # Linha de debug
                    try:
...
if __name__ == "__main__":
    ...
    if escolha == "1":
        resultado = atualizar_operacoes_rreo(status='now')
        print(resultado)
    elif escolha == "2":
